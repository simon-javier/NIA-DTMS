...
<?php 
    try {
        //code...
        $trackDocument = "SELECT tbl_admins_incoming.*, tbl_uploaded_document.*, tbl_admins_incoming.status as incoming_status from tbl_admins_incoming JOIN tbl_uploaded_document ON tbl_admins_incoming.docu_id = tbl_uploaded_document.id where tbl_admins_incoming.status = 'pending'";
        ...
        <tbody>
            <?php foreach ($results as $row) { ?>
                ...
                <td><img src="<?php echo $env_basePath; ?>assets/qr-codes/<?php echo $row['qr_filename']; ?>" alt="QR Code" style="height: 80px"></td>
                    ...
                    <td><a href="document-details.php?id=<?php echo $row['id']; ?>&source=incoming" class="btn btn-dark"><i class='bx bx-show'></i></a></td>
                </tr>
            <?php } ?>
